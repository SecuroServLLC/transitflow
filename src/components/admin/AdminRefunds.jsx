  ...
  const refund = useMutation({
    mutationFn: async ({ txn, customer }) => {
      ...
      await base44.entities.Transaction.create({
        ...
        amount: txn.amount, description: `Refund: ${txn.description}`, performed_by: 'admin'
      });
      return updated;
    },
    onSuccess: (updated) => {
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>{['Customer','Ticket ID','Type','Credits','Description','Date','Action'].map(h=><th key={h} className="text-left px-3 py-3 text-gray-600 font-medium text-xs">{h}</th>)}</tr>
          </thead>
          <tbody>
            {customerTxns.slice(0, 100).map(t => {
              return (
                <tr key={t.id} className={`border-b border-gray-100 last:border-0 ${t.refunded ? 'opacity-40' : ''}`}>
                  <td className="px-3 py-3 font-medium">{t.customer_name||'—'}</td>
                  <td className="px-3 py-3">
                    {t.ticket_id ? <code className="text-xs bg-gray-100 px-2 py-1 rounded font-mono">{t.ticket_id}</code> : <span className="text-gray-300">—</span>}
                  </td>
                  <td className="px-3 py-3 capitalize">
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                      ...
                      t.type==='bonus' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-500'
                    }`}>{t.type}</span>
                  </td>
                  <td className="px-3 py-3 font-bold">{t.amount > 0 ? `+${t.amount}` : t.amount} cr</td>
                  <td className="px-3 py-3 text-gray-500 max-w-xs truncate text-xs">{t.description}</td>
                  <td className="px-3 py-3 text-gray-400 text-xs">{new Date(t.created_date).toLocaleString('nb-NO')}</td>
                  <td className="px-3 py-3">
                    {!t.refunded && t.type === 'purchase' && t.customer_id && (
                      <Button size="sm" variant="outline"
                        onClick={async () => {
                          const list = await base44.entities.Customer.filter({ id: t.customer_id });
                          if (!list.length) { toast.error('Customer not found'); return; }
                    ...
                    {t.refunded && <span className="text-xs text-gray-400 italic">Refunded</span>}
                  </td>
                </tr>
              );
            })}
            {customerTxns.length===0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">No transactions</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
