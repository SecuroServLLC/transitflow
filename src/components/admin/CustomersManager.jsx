...
export default function CustomersManager() {
  ...
  const [creditsAmt, setCreditsAmt] = useState(0);
  ...
  const remove = useMutation({ mutationFn: id => base44.entities.Customer.delete(id), onSuccess: () => { qc.invalidateQueries({ queryKey: ['customers'] }); toast.success('Deleted'); } });
  ...
  const openCredits = c => { setSelected(c); setCreditsAmt(0); setModal('credits'); };
  ...
  const addCredits = () => update.mutate({ id: selected.id, d: { travel_credits: (selected.travel_credits || 0) + Number(creditsAmt) } });
  ...
  return (
    <div>
              ...
              <tr key={c.id} className="border-b border-gray-100 last:border-0">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-1">
                    <Button size="sm" variant="ghost" title="Give Credits" onClick={() => openCredits(c)}><Gift className="w-4 h-4 text-amber-500" /></Button>
                    <Button size="sm" variant="ghost" onClick={() => openEdit(c)}><Pencil className="w-4 h-4" /></Button>
                    <Button size="sm" variant="ghost" onClick={() => { if (confirm('Delete?')) remove.mutate(c.id); }}><Trash2 className="w-4 h-4 text-red-400" /></Button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <Dialog open={modal === 'form'} onOpenChange={() => setModal(null)}>
        <DialogContent><DialogHeader><DialogTitle>{selected ? 'Edit Customer' : 'Add Customer'}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            ...
            <div><Label>Travel Credits</Label><Input type="number" value={form.travel_credits} onChange={e => f('travel_credits', e.target.value)} /></div>
      ...
      <Dialog open={modal === 'credits'} onOpenChange={() => setModal(null)}>
        <DialogContent><DialogHeader><DialogTitle>Give Travel Credits — {selected?.name}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            ...
            <div><Label>Credits to Add</Label><Input type="number" min="0" value={creditsAmt} onChange={e => setCreditsAmt(e.target.value)} /></div>
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setModal(null)} className="flex-1">Cancel</Button>
