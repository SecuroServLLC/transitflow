...
export default function PricingManager() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (updates) => {
      await Promise.all(updates.map(u =>
        u.id
          ? base44.entities.Pricing.update(u.id, { credit_cost: u.credit_cost })
          : base44.entities.Pricing.create({ ticket_type: u.ticket_type, credit_cost: u.credit_cost })
      ));
    },
  ...
  const handleSave = () => {
    const updates = TYPES.map(t => ({
      id: prices[t.type]?.id,
      ticket_type: t.type,
      credit_cost: Number(prices[t.type]?.value ?? t.def),
    }));
        ...
        {TYPES.map(t => (
          <div key={t.type} className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div><h3 className="font-bold text-gray-900">{t.label}</h3><p className="text-xs text-gray-400">ticket type</p></div>
            </div>
