          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>{['Short Code','Type','Credits','Method','Status','Purchased','Used'].map(h => <th key={h} className="text-left px-4 py-3 text-gray-600 font-medium whitespace-nowrap">{h}</th>)}</tr>
              </thead>
              <tbody>
                {tickets.map(t => (
                  <tr key={t.id} className="border-b border-gray-100 last:border-0">
                    <td className="px-4 py-3 font-mono text-xs font-bold text-gray-700">{t.short_code}</td>
                    ...
                    <td className="px-4 py-3 text-gray-400 whitespace-nowrap">{t.purchased_at ? new Date(t.purchased_at).toLocaleString() : '-'}</td>
                    <td className="px-4 py-3 text-gray-400 whitespace-nowrap">{t.used_at ? new Date(t.used_at).toLocaleString() : '-'}</td>
                  </tr>
                ))}
