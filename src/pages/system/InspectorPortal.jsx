...
export default function InspectorPortal() {
  ...
  const [fineForm, setFineForm] = useState({
    name: '', dob: '', ssn: '', phone: '', email: '',
    address: '', zip: '', city: '',
    reason: 'No Valid Ticket', busline: '', tripID: ''
  });
  ...
  const forceOverride = useMutation({
    mutationFn: ({ id, reason }) => base44.entities.Ticket.update(id, {
      status: 'used', used_at: new Date().toISOString(),
      notes: `OVERRIDE by ${inspector.name}: ${reason}`
    }),
    onSuccess: (updated) => {
      toast.success('Manual override applied');
      addLog(updated, 'Override');
      setResult({ valid: true, ticket: updated, msg: `Override: ${overrideReason}` });
      setOverrideReason('');
  ...
  const issueFine = useMutation({
    mutationFn: data => base44.entities.Fine.create({
      ...data,
      issued_by: inspector.name,
    ...
    onSuccess: () => {
      toast.success('Digital fine issued!');
      setFineForm({ name: '', dob: '', ssn: '', phone: '', email: '', address: '', zip: '', city: '', reason: 'No Valid Ticket', busline: '', tripID: '' });
      setPanel('scan');
    }
  });

  const loginBadge = () => {
    if (!badgeId.trim()) { toast.error('Enter your Badge ID'); return; }
    const found = inspectors.find(i =>
      i.badge_id?.toLowerCase() === badgeId.trim().toLowerCase() && i.is_active !== false
    );
    if (!found) { toast.error('Badge ID not found or account inactive'); return; }
    setStep('pin');
  };
  ...
  const validate = () => {
    ...
    const ticket = tickets.find(t =>
      ...
      t.qr_token?.substring(0, 8).toUpperCase() === q
    );
    ...
    if (ticket.ticket_category === 'period') {
      ...
      if (!validUntil || validUntil < now) {
        setResult({ valid: false, reason: 'Period pass expired', ticket });
        addLog(ticket, 'Expired');
        return;
      }
      setResult({ valid: true, ticket, msg: `Period pass — valid until ${validUntil.toLocaleDateString()}` });
      addLog(ticket, 'Valid');
      return;
    }
  ...
  // LOGIN — Badge step
  if (!inspector && step === 'badge') return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4">
      <div className="bg-[#111] border border-slate-800 rounded-2xl p-8 w-full max-w-sm space-y-6">
        <div className="text-center">
          ...
          <h1 className="text-xl font-black text-white">Inspector Portal</h1>
          <p className="text-slate-500 text-sm mt-1">Enter your Badge ID</p>
        </div>
        <Input
          placeholder="Badge ID (e.g. INS-001)"
          value={badgeId}
          onChange={e => setBadgeId(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && loginBadge()}
          className="bg-[#0a0a0a] border-slate-700 text-white h-12 text-center text-lg font-mono tracking-widest"
        />
        <Button onClick={loginBadge} className="w-full h-12 bg-[#c0392b] hover:bg-[#a93226] font-bold">
          Next →
        </Button>
      </div>
    </div>
  ...
  // LOGIN — PIN step
  if (!inspector && step === 'pin') return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4">
      <div className="bg-[#111] border border-slate-800 rounded-2xl p-8 w-full max-w-sm space-y-6">
        <div className="text-center">
          <div className="text-4xl mb-3">🔐</div>
          <h1 className="text-xl font-black text-white">Enter PIN</h1>
          <p className="text-slate-400 text-sm">Badge: <span className="text-[#c0392b] font-bold font-mono">{badgeId.toUpperCase()}</span></p>
        </div>
        <Input
          type="password"
          placeholder="PIN"
        ...
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => { setStep('badge'); setPin(''); }} className="flex-1 border-slate-700 text-slate-300">← Back</Button>
          <Button onClick={loginPin} className="flex-1 h-12 bg-[#c0392b] hover:bg-[#a93226]">Login</Button>
        </div>
      </div>
    </div>
  );
      ...
      <aside className="w-full md:w-72 bg-[#111] border-b md:border-b-0 md:border-r border-slate-800 flex flex-col">
        <div className="p-4 border-b border-slate-800 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <LSTLogo size={28} />
            <div>
              <p className="font-bold text-sm text-white">{inspector.name}</p>
              <p className="text-slate-500 text-xs font-mono">{inspector.badge_id}</p>
            </div>
          </div>
        ...
        <div className="p-3 flex gap-2">
          <button
            onClick={() => { setPanel('scan'); setResult(null); }}
            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-colors ${panel === 'scan' ? 'bg-[#c0392b] text-white' : 'bg-slate-800 text-slate-400'}`}
          >📋 Scan</button>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          <p className="text-[10px] text-slate-600 font-bold tracking-wider uppercase flex items-center gap-1">
            <Clock className="w-3 h-3" /> Session Log ({localScans.length})
          </p>
          {localScans.length === 0
            ? <p className="text-slate-700 text-xs text-center py-6">No scans yet</p>
            : localScans.map((l, i) => (
              <div key={i} className="bg-[#0a0a0a] border border-slate-900 rounded-lg p-2 text-xs">
                ...
                <div className="flex justify-between mt-0.5">
                  <span className="text-slate-500 capitalize">{l.type}</span>
                  <span className={`font-bold text-[10px] ${l.status === 'Valid' ? 'text-green-400' : l.status === 'Override' ? 'text-orange-400' : 'text-red-400'}`}>{l.status}</span>
                </div>
              </div>
            ))
          }
        ...
        {panel === 'scan' ? (
          <div className="w-full max-w-md space-y-6">
            {!result ? (
              <div className="text-center space-y-5">
                ...
                <Input
                  placeholder="Short Code or Ticket ID"
                  value={code}
                  onChange={e => setCode(e.target.value.toUpperCase())}
                  onKeyDown={e => e.key === 'Enter' && validate()}
                  className="bg-[#111] border-slate-700 text-white text-center text-xl font-mono h-16 tracking-widest"
                />
                  ...
                  <div className="bg-red-950/40 border-2 border-red-500 rounded-3xl p-8 space-y-3">
                    ...
                    {result.ticket && (
                      <div className="border-t border-slate-700 pt-4 space-y-2 text-left">
                        ...
                        />
                        <Button
                          onClick={() => forceOverride.mutate({ id: result.ticket.id, reason: overrideReason })}
                          disabled={forceOverride.isPending || !overrideReason.trim()}
                          className="w-full bg-orange-600 hover:bg-orange-700 text-sm font-bold"
          ...
          <div className="w-full max-w-lg bg-[#111] border border-red-900/30 rounded-2xl p-6 space-y-4">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h2 className="text-xl font-black text-white">Digital Penalty Fare</h2>
                <p className="text-slate-500 text-xs">Fine of 1,150 kr will be issued</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                <Label className="text-xs text-slate-400">Trip ID</Label>
                <div className="flex gap-1 mt-1">
                  <Input value={fineForm.tripID} readOnly className="bg-[#0a0a0a] border-slate-700 text-white text-xs font-mono flex-1" />
                  <Button size="sm" onClick={genTripID} className="bg-slate-700 hover:bg-slate-600 text-xs">Gen</Button>
                </div>
              </div>
            ...
            <Button
              onClick={() => {
                if (!fineForm.name || !fineForm.phone || !fineForm.tripID) { toast.error('Name, Phone, and Trip ID are required'); return; }
                issueFine.mutate(fineForm);
              }}
              disabled={issueFine.isPending}
              className="w-full bg-red-700 hover:bg-red-800 font-bold h-12"
            >
